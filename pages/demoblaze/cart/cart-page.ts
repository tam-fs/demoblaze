...
export class CartPage extends CommonPage {
  ...
  async navigateToHomePage(): Promise<void> {
    await this.page.goto(process.env.DEMOBLAZE_URL || "https://www.demoblaze.com");
  }
  ...
  async removeItem(productName: string): Promise<void> {
    ...
    const row = this.locators.cartItemRowByProductName(productName);
  ...
  async clearCart(): Promise<void> {
    await this.waitForVisible(this.locators.cartTable);
    const deleteButtons = this.locators.deleteButton;
  ...
  @step("Verify cart cleared")
  async verifyCleared(): Promise<void> {
    const items = await this.getCartItems();
    expect.soft(items.length).toBe(0);
  }
}

...
export function loadEnvironment(envArg?: string): LoadResult {
	...
	const envName = (envArg || process.env.TEST_ENV || process.env.ENV || process.env.npm_config_env || cliArg || 'stg').toLowerCase();
	...
	return { envName, error: 'No .env file found for candidates: ' + candidates.join(', ') };
}
...
export default async function globalSetup() {
	...
	if (res.loadedPath) {
		...
		console.log(`[global-setup] Loaded env '${res.envName}' from: ${res.loadedPath}`);
	} else {
		// eslint-disable-next-line no-console
		console.warn(`[global-setup] Did not find an env file for '${res.envName}'. ${res.error || ''}`);
	}
